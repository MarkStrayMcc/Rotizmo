...
using TemplateConfig = Hero.Integration.Aspose.MultipleProperties.LifeSciences.TemplateConfiguration;

namespace Hero.Integration.Aspose.MultipleProperties.LifeSciences;

internal sealed partial class LifeSciencesOrchestrator : BaseMultiplePropertyOrchestrator<LifeSciencesUploadContext>
{
    private const string BuildingUseTag = "BuildingUse";
    private const int MaximumLocationTivValue = 300000000;

    private readonly ILogger<LifeSciencesOrchestrator> _logger;
    ...
    public LifeSciencesOrchestrator(
        ILogger<LifeSciencesOrchestrator> logger,
    ...
    protected override async Task<LifeSciencesUploadContext> ExtractContextAsync(
        Workbook workbook,
        List<ValidationResult> validationResults)
    {
        ...
        return new LifeSciencesUploadContext
        {
    ...
    protected override async Task<List<PropertyLimit>> BuildPropertyLimitsAsync(
        ...
        LifeSciencesUploadContext context,
        List<ValidationResult> validationResults)
    {
        ...
        foreach (var firstSheetExtractedRow in firstSheetExtractedRows)
        {
            ...
            try
            {
                propertyLimit = new PropertyLimit
                {
            ...
            catch (InvalidBuildingUseException)
            {
                validationResults.Add(new ValidationResult
                {
                    ...
                    {
                        new() { ErrorMessage = "Invalid building use value" }
                    }
                });
                continue;
            }
            ...
            if ((propertyLimit.TotalInsuredValue + propertyTotalFloatingValues) > MaximumLocationTivValue
                && (context.FirstLossLimit == 0 || context.FirstLossLimit > MaximumLocationTivValue))
            {
                validationResults.Add(new ValidationResult
                {
                    LocationIdentifier = "Row " + firstSheetExtractedRow.RowNumber,
                    ...
                    Errors = new List<FluentValidation.Results.ValidationFailure>
                    {
                        new() { ErrorMessage = $"Location maximum limit of £{MaximumLocationTivValue:N0} has been exceeded" }
                    }
                });
                continue;
            }
