...
internal sealed partial class TerrorismOrchestrator : BaseMultiplePropertyOrchestrator<TerrorismUploadContext>
{
    ...
    protected override Task PostProcessPropertyLimitsAsync(List<PropertyLimit> propertyLimits, List<ValidationResult> validationResults)
    {
        return _priorSubmitApprovalService.EvaluateLocationsAsync(propertyLimits);
    }
    ...
    public override async Task<Workbook> InsertPropertyLimitsIntoWorkbook(
        Workbook workbook,
        ...
        int wordingVersionId)
    {
        ...
        AsposeRowInserter.InsertRows(workbook, locationRows, locationExtractorColumnConfiguration, 0);
        AsposeRowInserter.InsertRows(workbook, floatingValueRows, floatingValuesExtractorColumnConfiguration, 1);
        AsposeRowInserter.InsertRows(workbook, firstLossValueRows, firstLossLimitExtractorColumnConfiguration, 2);
    ...
    {
        if (thirdSheetExtractedRows?.Count == 0)
            return 0;
    ...
    private async Task<PropertyLimitFloatingValues?> SetExtractedPropertyFloatingValues(
        List<ExtractedRow> secondSheetExtractedRows,
        List<ValidationResult> validationResults)
    {
        if (secondSheetExtractedRows.Count == 0)
            return null;

        var secondSheetExtractedRow = secondSheetExtractedRows.FirstOrDefault();
        var validationResult = await _floatingValueValidator.ValidateAsync(
            new FloatingValueValidationRequest { ExtractedFloatingValueRow = secondSheetExtractedRow });
    ...
    private static LocationValidationRequest CreateLocationRequestFrom(ExtractedRow extractedRow)
    {
        ...
        {
            CountryName = row[TemplateConfig.Country].StringValue,
            Address1 = row[TemplateConfig.AddressLine1].StringValue,
            Address2 = row[TemplateConfig.AddressLine2].StringValue,
            Address3 = null,
            City = row[TemplateConfig.TownOrCity].StringValue,
            County = row[TemplateConfig.County].StringValue,
            StateProvince = row[TemplateConfig.StateOrProvince].StringValue,
            Postcode = row[TemplateConfig.PostCode].StringValue,
            Latitude = row[TemplateConfig.Latitude].DoubleValue,
            Longitude = row[TemplateConfig.Longitude].DoubleValue,
            BuildingUse = row[TemplateConfig.BuildingUse].StringValue
        };
    }

    private static ClientLocationProperties? AddNewLocationAttributeFor(string locationAttributeValue, string tag)
    {
        if (string.IsNullOrEmpty(locationAttributeValue))
            return null;

        var buildingUse = locationAttributeValue.Trim().ToUpperInvariant();
        if (Enum.IsDefined(typeof(BuildingUseTypes), buildingUse))
        {
            return new ClientLocationProperties
            {
                Tag = tag,
                Value = buildingUse
            };
    ...
    private async Task<ExtractedCellValue?> GetLocationCellValue(WebApiDto.Dto.PropertyLimit propertyLimit, ExtractorColumn column)
    {
        ...
        switch (column.ColumnName)
        {
            ...
            case TemplateConfig.StateOrProvince:
                var stateProvinceName = await GetStateProvinceName(
                    propertyLimit.InsuredAddress.StateProvinceCode,
    ...
    private static ExtractedCellValue? GetFloatingValueCellValue(
        PropertyLimitFloatingValues propertyLimitFloatingValues,
        ExtractorColumn column)
    {
        ExtractedCellValue cellValue = new();
        cellValue.ColumnName = column.ColumnName;
    ...
    private static ExtractedCellValue? GetFirstLossLimitCellValue(int? firstLossLimit, ExtractorColumn column)
    {
        ExtractedCellValue cellValue = new();
        cellValue.ColumnName = column.ColumnName;
    ...
    private bool IsZoneAAddress(WebApiDto.Dto.ClientLocation clientLocation)
    {
        ...
        var outCode = whitespaceRemovedPostcode.Length >= ukPostCodeLength
            ? whitespaceRemovedPostcode[..^ukInCodeLength]
            : whitespaceRemovedPostcode;

        var outCodeTrailingAlphabeticalCharacterRemoved =
            _zoneAOutCodeRegex.IsMatch(outCode)
                ? _zoneAOutCodeRegex.Matches(outCode).Single().Value
                : string.Empty;
    ...
    [LoggerMessage(
        EventId = 1,
        Level = LogLevel.Error,
        Message = "Template does not contain any worksheets; filename: {TemplateFilename}")]
